import { Client as FugitiveKingClient, type Game } from './bindings';
...
/**
 * Service for interacting with the FugitiveKing game contract
 */
export class FugitiveKingService {
  private baseClient: FugitiveKingClient;
  private contractId: string;

  constructor(contractId: string) {
    this.contractId = contractId;
    // Base client for read-only operations
    this.baseClient = new FugitiveKingClient({
  ...
  private createSigningClient(
    ...
    return new FugitiveKingClient(options);
  }
  ...
  async prepareStartGame(
    ...
    const buildClient = new FugitiveKingClient({
      contractId: this.contractId,
      networkPassphrase: NETWORK_PASSPHRASE,
      rpcUrl: RPC_URL,
...
// Example: const fugitiveKingService = new FugitiveKingService(contractId);
